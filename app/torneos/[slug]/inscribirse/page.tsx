...
import { getTournamentBySlug, getOrganizer, whatsappLink } from '@/lib/tournaments';
...
export default async function TournamentSignupPage({ params }: Props) {
  const { slug } = await params;
  const tournament = await getTournamentBySlug(slug);
  if (!tournament) notFound();

  if (tournament.status !== 'open') {
    // Si el torneo no está abierto a inscripción, mandamos a la página pública.
    redirect(`/torneos/${slug}`);
  }

  const user = await getUser();
  if (!user) {
    redirect(`/auth/login?next=${encodeURIComponent(`/torneos/${slug}/inscribirse`)}`);
  }
      ...
      <SubscribeGate>
        <RulesGate>
          <div className="max-w-2xl mx-auto mb-8">
            <header className="text-center">
              ...
              <p className="text-white/60 text-sm mt-3 max-w-md mx-auto">
                Logueado como <span className="text-amber-gold">{captainName}</span>.
              </p>
                ...
                <div className="mt-6 inline-flex flex-col items-center gap-1 border border-amber-gold/30 bg-amber-gold/5 px-5 py-3">
                  ...
                  {Number(tournament.entry_fee_per_player_usd) > 0 && (
                    <div className="font-mono text-[11px] text-white/55 tracking-[0.18em]">
                      {tournament.team_size} jugadores × USD $
                      {Number(tournament.entry_fee_per_player_usd).toFixed(0)}/jugador
                    </div>
                  )}
                </div>
              )}
          ...
          <RegisterWizard
            ...
            entryFeePerTeamUsd={Number(tournament.entry_fee_per_team_usd)}
            entryFeePerPlayerUsd={Number(tournament.entry_fee_per_player_usd)}
